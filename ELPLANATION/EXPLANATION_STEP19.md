...
```mermaid
flowchart TD
    A[push または pull_request] --> B{変更対象に<br/>backend/** または<br/>.github/workflows/backend-ci.yml を含むか}
    B -- No --> Z[workflow は起動しない]
    B -- Yes --> C[Backend CI workflow 開始]
    C --> D[actions/checkout で repository を取得]
    D --> E[actions/setup-python で Python 3.13 を準備]
    E --> F[pip install -r requirements.txt]
    F --> G[pip install -e .]
    G --> H[DATABASE_URL を test 用に設定]
    H --> I[ruff check .]
    I --> J{lint 成功?}
    J -- No --> JF[Run Ruff check で失敗]
    J -- Yes --> K[ruff format --check .]
    K --> L{format check 成功?}
    L -- No --> LF[Run Ruff format check で失敗]
    L -- Yes --> M[python -m pytest]
    M --> N{pytest 成功?}
    N -- No --> NF[Run pytest で失敗]
    N -- Yes --> O[backend-quality job 成功]
```

この図で何が分かるか:

- workflow の入口は `push` または `pull_request` です
- ただし毎回起動するのではなく、`backend/**` または workflow 自身が変わったときだけ動きます
- job の中では、repository 取得、Python 準備、依存関係 install、editable install、lint、format check、pytest の順で進みます
- `DATABASE_URL` は import 初期化を通すための前提として job 開始時点で設定されます
- 失敗時は `Run Ruff check` `Run Ruff format check` `Run pytest` のどこで止まったかで切り分けます
