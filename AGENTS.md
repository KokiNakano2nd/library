...
Markdown、Python、TypeScript、TSX、JSON、YAMLなどのテキストファイルは、原則としてUTF-8で保存する

PowerShellで日本語を含むファイルを読む場合は、`Get-Content -Encoding UTF8` を優先して利用する

日本語テキストが文字化けした場合は、まずファイルの実体を変更せず、UTF-8指定での再読、BOM有無の確認、必要に応じて `Default` など別エンコーディングでの再読を行い、表示系の問題かファイル実体の問題かを切り分ける

文字コードを変換して保存する場合は、文字化けを直すこと自体を目的に安易に上書きせず、対象ファイルの利用者と既存ツールの前提を確認したうえで、UTF-8へ統一する方針を優先する

新規作成ファイルはUTF-8を前提とし、既存ファイルがUTF-8以外で管理されていることが判明した場合は、その理由を確認するまで同一ファイル内で文字コードを混在させない
...
GitHub Actions の workflow 自体の説明を独立して残す場合は、ELPLANATION/workflow 配下に格納する
