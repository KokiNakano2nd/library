...
- ローカル開発ではNext.jsの `3000` 番、Playwright確認ではNext.jsの `3011` 番と isolated run 用の `3012` 番からFastAPIへのCORS通信を許可する
...
| URL | 画面名 | 主な機能 |
| --- | --- | --- |
| `/books` | 本の一覧画面 | ログイン済み利用者向けの表形式一覧表示、登録画面・編集画面への移動、削除 |
| `/login` | ログイン画面 | `login_id` と `password` を入力して認証する |
| `/books/new` | 本の新規登録画面 | 入力優先度を示すガイドと補助説明付きフォームから本を登録 |
| `/books/[id]/edit` | 本の編集画面 | 登録済みの本を取得して更新 |

トップページ `/` は `/books` へ誘導またはリダイレクトします。

削除専用画面は作りません。一覧画面は `shadcn/ui` の `Table` をベースに表示し、削除ボタンを押した際は `Dialog` 上で対象タイトルを確認し、承認された場合だけ削除します。
...
`/books/new` は、左側に入力ガイド、右側に登録フォームを置く 2 カラムレイアウトを基本とし、mobile では 1 カラムへ切り替える方針とします。フォームでは必須・任意の区別、補助テキスト、フィールド単位のエラー表示、送信中の状態表示を行います。

frontend では、未ログイン時だけ `/login` へリダイレクトし、ログイン済みであれば登録・編集・削除の導線を同じ画面構成で表示します。実際の更新系操作の可否は backend の認可結果で判定します。
...
- frontend では `/books` `/books/new` `/books/[id]/edit` をログイン済み利用者向け画面とし、未ログイン時だけ `/login` へリダイレクトします
- 登録・更新・削除の最終的な許可判定は backend が行い、frontend では role ごとに別画面を出し分けません
...
- `components/ui`: `shadcn/ui` ベースの共通 UI 部品
...
- `lib/utils.ts`: `shadcn/ui` で使う `className` 結合 helper
...
`NEXT_PUBLIC_API_BASE_URL` は browser 向けの既定値として残しますが、browser 実行時の API 呼び出しは最終的に `window.location.hostname` を使って `http://<現在のホスト名>:8000` を組み立てます。
これにより、`localhost:3000` でも `127.0.0.1:3000` でも、ログインCookieの保存先ホスト名と API 呼び出し先ホスト名をそろえます。
